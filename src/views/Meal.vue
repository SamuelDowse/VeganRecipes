<template>
  <v-row class="text-left ma-6" v-if="recipe.name">
    <v-col md="4">
      ...
      <div v-for="item in recipe.ingredientsOrder" v-bind:key="item.name">
        ...
        <ul>
          <li
            v-for="subItem in recipe.ingredients[item]"
            :key="subItem.name"
            v-html="subItem"
          ></li>
        </ul>
    ...
    <v-col md="8">
      ...
      <v-btn style="margin:10px;" class="success" @click="readRecipe()">{{
        readRecipeButton
      }}</v-btn>
      <v-btn
        style="margin:10px;"
      ...
      >
      <p
        v-for="item in recipe.stepsOrder"
        v-bind:key="item.name"
        style="padding: 10px 0px;"
        v-html="'<b>' + item + '</b>: ' + recipe.steps[item]"
      ></p>
    </v-col>
  </v-row>
</template>
...
export default {
  ...
  methods: {
    readRecipe() {
      ...
      utter.text = this.recipe.steps[
        this.recipe.stepsOrder[this.instructionCount]
      ];
