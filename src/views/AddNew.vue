<template>
  <div class="mx-6">
    ...
    >
      <v-list>
        <v-list-item v-for="(item, index) in items" :key="index">
          <v-list-item-title style="cursor:pointer;" @click="doAction">{{
            item.title
          }}</v-list-item-title>
        </v-list-item>
      </v-list>
    ...
    <v-alert
      ...
      dismissible
      style="position: fixed; bottom: 50px; right: 20px; z-index:100;"
    >
            ...
            <v-col md="12">
              <v-sheet style="padding: 20px;">
                ...
                <v-row>
                  <v-col md="12">
                    <v-data-table
                      :headers="[{ text: 'Setup Step', value: 'setupStep' }]"
...
export default {
  ...
  data: () => ({
    ...
    x: 0,
    y: 0,
    items: [{ title: "Move Up" }, { title: "Move Down" }, { title: "Delete" }],
    // -------
    categories: [],
    ingredients: {},
    ingredientsOrder: [],
  ...
  methods: {
    addNewRecipe() {
      ...
      for (i = 0; i < this.tempIngredients.length; i++) {
        if (
          this.ingredientsOrder.includes(this.tempIngredients[i].parent) ==
          false
        ) {
